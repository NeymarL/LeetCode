...
map<tuple<int, int>, bool> result;
bool can_cross(vector<int>& stones, int k, int index);
...
bool can_cross(vector<int>& stones, int k, int index)
{
    if (index + 1 == stones.size()) {
        result[make_tuple(k, index)] = true;
        return true;
    }
    if (stones.back() - stones[index] <= k + 1 && stones.back() - stones[index] >= k - 1) {
        result[make_tuple(k, index)] = true;
        return true;
    }
    for (int i = index + 1; i < stones.size(); i++) {
        ...
        if (distance <= k + 1 && distance >= k - 1) {
            if (result.find(make_tuple(distance, i)) != result.end()) {
                return result[make_tuple(distance, i)];
            }
            else if (can_cross(stones, distance, i)) {
                result[make_tuple(k, index)] = true;
                return true;
            }
        } 
    ...
    result[make_tuple(k, index)] = false;
    return false;
}
